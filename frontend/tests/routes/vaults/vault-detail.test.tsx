...
vi.mock("../../../src/services/apis/rest-api/trade.service", () => ({
...
vi.mock("../../../src/stores", () => ({
  useWebSocketStore: (selector: (state: Record<string, unknown>) => unknown) =>
    selector({
...
vi.mock("../../../src/components/ui/SolscanLink", () => ({
  SolscanLink: ({ signature }: { signature: string }) => (
...
describe("VaultOverview", () => {
  it("renders metadata, fees, assets, and description with edit link when manager", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Investment Strategy/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /Protocol Parameters/i }),
    ).toBeInTheDocument();
    expect(screen.getByText("15.00% Perf / 2.00% Mgmt")).toBeInTheDocument();
    expect(screen.getByText("SOL")).toBeInTheDocument();
    expect(
      screen.getByText("A diversified Solana strategy."),
    ).toBeInTheDocument();
  ...
  it("renders empty assets state and edit link when manager", () => {
    ...
    expect(
      screen.getByText("All whitelisted ecosystem tokens allowed."),
    ).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Edit/i })).toHaveAttribute(
      "href",
      "/vaults/vault-1/edit",
...
describe("VaultTradesTab", () => {
  ...
  it("renders formatted trades and transaction link", async () => {
    ...
    expect(screen.getByRole("link", { name: "Solscan" })).toHaveAttribute(
      "href",
