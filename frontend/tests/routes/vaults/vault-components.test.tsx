...
const mocks = vi.hoisted(() => ({ sparkline: [1, 2, 3] as number[] | undefined }))

vi.mock('@tanstack/react-router', () => ({
  Link: ({ children, to, params, search }: { children: React.ReactNode; to: string; params?: { id: string }; search?: { vaultId: string } }) => {
    const href = params ? to.replace('$id', params.id) : search ? `${to}?vaultId=${search.vaultId}` : to
...
describe('VaultsTable', () => {
  beforeEach(() => { mocks.sparkline = [1, 2, 3] })
  ...
  it.each(['Enter', ' '])('sorts by %s keyboard activation', (key) => {
    const onSort = vi.fn()
    render(<VaultsTable vaults={[makeVault()]} onSort={onSort} />)
    fireEvent.keyDown(screen.getByRole('columnheader', { name: /CREATED/ }), { key })
    expect(onSort).toHaveBeenCalledWith('created_at')
  })

  it('exposes active sort direction', () => {
    render(<VaultsTable vaults={[makeVault()]} sortBy="pnl" sortOrder="asc" onSort={vi.fn()} />)
    expect(screen.getByRole('columnheader', { name: /PNL/ })).toHaveAttribute('aria-sort', 'ascending')
  })
  ...
  it('renders sparkline fallback when query has no points', () => {
    ...
    const { container } = render(<VaultsTable vaults={[makeVault()]} onSort={vi.fn()} />)
    expect(container.querySelector('line')).toHaveAttribute('stroke-dasharray', '1 3')
...
describe('VaultCard', () => {
  ...
  it('renders edit, trade, and view destinations', () => {
    ...
    expect(screen.getByRole('link', { name: 'Trade' })).toHaveAttribute('href', '/trade?vaultId=vault-1')
    expect(screen.getByRole('link', { name: 'View' })).toHaveAttribute('href', '/vaults/vault-1')
  })

  it('uses ID fallback title', () => {
    render(<VaultCard vault={makeVault({ metadata: { displayName: '', description: '', focusAssets: [] } })} />)
    expect(within(screen.getByText('Vault vault-1').closest('div')!).getByText('Active')).toBeInTheDocument()
  })
})
