...
vi.mock('../../../src/stores', () => ({
  usePortfolioStore: (selector: (state: { positions: unknown[] }) => unknown) => selector({ positions: mocks.positions }),
}))
...
describe('InvestorVaultsList', () => {
  ...
  it('renders empty state and explore link', () => {
    ...
    expect(screen.getByRole('link', { name: 'Explore Vaults' })).toHaveAttribute('href', '/vaults')
  })
  ...
  it('sorts by current value by default', () => {
    mocks.positions = [
      makePosition({ vaultId: 'low', vaultName: 'Low Value', currentValue: 100, pnlPercent: 90 }),
      makePosition({ vaultId: 'high', vaultName: 'High Value', currentValue: 500, pnlPercent: 1 }),
    ]
    render(<InvestorVaultsList />)
  ...
  it('toggles sorting to PNL', () => {
    mocks.positions = [
      makePosition({ vaultId: 'low', vaultName: 'Low PNL', currentValue: 500, pnlPercent: 1 }),
      makePosition({ vaultId: 'high', vaultName: 'High PNL', currentValue: 100, pnlPercent: 90 }),
    ]
    render(<InvestorVaultsList />)
    fireEvent.click(screen.getByRole('button', { name: 'Sort: Value' }))
    const rows = screen.getAllByRole('row').slice(1)
    expect(within(rows[0]).getByText('High PNL')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Sort: PNL' })).toBeInTheDocument()
  })
})

describe('ManagerVaultsList', () => {
  beforeEach(() => { mocks.vaultQuery = { data: [], isLoading: false } })
  ...
  it('filters managed vaults case-insensitively', () => {
    mocks.vaultQuery.data = [
      makeVault({ id: 'mine', managerAddress: 'ABC', metadata: { displayName: 'Mine', description: '', focusAssets: [] } }),
      makeVault({ id: 'other', managerAddress: 'XYZ', metadata: { displayName: 'Other', description: '', focusAssets: [] } }),
    ]
  ...
  it('renders populated manager metrics and details link', () => {
    ...
    expect(screen.getAllByRole('link').some((link) => link.getAttribute('href') === '/vaults/vault-1')).toBe(true)
  })
})
