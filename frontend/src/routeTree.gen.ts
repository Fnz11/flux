/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file was automatically generated by TanStack Router.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { Route as rootRouteImport } from './routes/__root'
import { Route as IndexRouteImport } from './routes/index'
import { Route as PayoutRouteImport } from './routes/payout'
import { Route as SettingsRouteImport } from './routes/settings'
import { Route as TradeRouteImport } from './routes/trade'
import { Route as InvestIndexRouteImport } from './routes/invest/index'
import { Route as PortfolioIndexRouteImport } from './routes/portfolio/index'
import { Route as VaultsIndexRouteImport } from './routes/vaults/index'
import { Route as VaultsCreateRouteImport } from './routes/vaults/create'
import { Route as InvestVaultsIndexRouteImport } from './routes/invest/vaults/index'
import { Route as VaultsIdIndexRouteImport } from './routes/vaults/$id/index'
import { Route as VaultsIdEditRouteImport } from './routes/vaults/$id/edit'
import { Route as InvestVaultsIdIndexRouteImport } from './routes/invest/vaults/$id/index'

const IndexRoute = IndexRouteImport.update({
  id: '/',
  path: '/',
  getParentRoute: () => rootRouteImport,
} as any)
const PayoutRoute = PayoutRouteImport.update({
  id: '/payout',
  path: '/payout',
  getParentRoute: () => rootRouteImport,
} as any)
const SettingsRoute = SettingsRouteImport.update({
  id: '/settings',
  path: '/settings',
  getParentRoute: () => rootRouteImport,
} as any)
const TradeRoute = TradeRouteImport.update({
  id: '/trade',
  path: '/trade',
  getParentRoute: () => rootRouteImport,
} as any)
const InvestIndexRoute = InvestIndexRouteImport.update({
  id: '/invest/',
  path: '/invest/',
  getParentRoute: () => rootRouteImport,
} as any)
const PortfolioIndexRoute = PortfolioIndexRouteImport.update({
  id: '/portfolio/',
  path: '/portfolio/',
  getParentRoute: () => rootRouteImport,
} as any)
const VaultsIndexRoute = VaultsIndexRouteImport.update({
  id: '/vaults/',
  path: '/vaults/',
  getParentRoute: () => rootRouteImport,
} as any)
const VaultsCreateRoute = VaultsCreateRouteImport.update({
  id: '/vaults/create',
  path: '/vaults/create',
  getParentRoute: () => rootRouteImport,
} as any)
const InvestVaultsIndexRoute = InvestVaultsIndexRouteImport.update({
  id: '/invest/vaults/',
  path: '/invest/vaults/',
  getParentRoute: () => rootRouteImport,
} as any)
const VaultsIdIndexRoute = VaultsIdIndexRouteImport.update({
  id: '/vaults/$id/',
  path: '/vaults/$id/',
  getParentRoute: () => rootRouteImport,
} as any)
const VaultsIdEditRoute = VaultsIdEditRouteImport.update({
  id: '/vaults/$id/edit',
  path: '/vaults/$id/edit',
  getParentRoute: () => rootRouteImport,
} as any)
const InvestVaultsIdIndexRoute = InvestVaultsIdIndexRouteImport.update({
  id: '/invest/vaults/$id/',
  path: '/invest/vaults/$id/',
  getParentRoute: () => rootRouteImport,
} as any)

export interface FileRoutesByFullPath {
  '/': typeof IndexRoute
  '/payout': typeof PayoutRoute
  '/settings': typeof SettingsRoute
  '/trade': typeof TradeRoute
  '/vaults/create': typeof VaultsCreateRoute
  '/invest/': typeof InvestIndexRoute
  '/portfolio/': typeof PortfolioIndexRoute
  '/vaults/': typeof VaultsIndexRoute
  '/vaults/$id/edit': typeof VaultsIdEditRoute
  '/invest/vaults/': typeof InvestVaultsIndexRoute
  '/vaults/$id/': typeof VaultsIdIndexRoute
  '/invest/vaults/$id/': typeof InvestVaultsIdIndexRoute
}
export interface FileRoutesByTo {
  '/': typeof IndexRoute
  '/payout': typeof PayoutRoute
  '/settings': typeof SettingsRoute
  '/trade': typeof TradeRoute
  '/vaults/create': typeof VaultsCreateRoute
  '/invest': typeof InvestIndexRoute
  '/portfolio': typeof PortfolioIndexRoute
  '/vaults': typeof VaultsIndexRoute
  '/vaults/$id/edit': typeof VaultsIdEditRoute
  '/invest/vaults': typeof InvestVaultsIndexRoute
  '/vaults/$id': typeof VaultsIdIndexRoute
  '/invest/vaults/$id': typeof InvestVaultsIdIndexRoute
}
export interface FileRoutesById {
  __root__: typeof rootRouteImport
  '/': typeof IndexRoute
  '/payout': typeof PayoutRoute
  '/settings': typeof SettingsRoute
  '/trade': typeof TradeRoute
  '/vaults/create': typeof VaultsCreateRoute
  '/invest/': typeof InvestIndexRoute
  '/portfolio/': typeof PortfolioIndexRoute
  '/vaults/': typeof VaultsIndexRoute
  '/vaults/$id/edit': typeof VaultsIdEditRoute
  '/invest/vaults/': typeof InvestVaultsIndexRoute
  '/vaults/$id/': typeof VaultsIdIndexRoute
  '/invest/vaults/$id/': typeof InvestVaultsIdIndexRoute
}
export interface FileRouteTypes {
  fileRoutesByFullPath: FileRoutesByFullPath
  fullPaths:
    | '/'
    | '/payout'
    | '/settings'
    | '/trade'
    | '/vaults/create'
    | '/invest/'
    | '/portfolio/'
    | '/vaults/'
    | '/vaults/$id/edit'
    | '/invest/vaults/'
    | '/vaults/$id/'
    | '/invest/vaults/$id/'
  fileRoutesByTo: FileRoutesByTo
  to:
    | '/'
    | '/payout'
    | '/settings'
    | '/trade'
    | '/vaults/create'
    | '/invest'
    | '/portfolio'
    | '/vaults'
    | '/vaults/$id/edit'
    | '/invest/vaults'
    | '/vaults/$id'
    | '/invest/vaults/$id'
  id:
    | '__root__'
    | '/'
    | '/payout'
    | '/settings'
    | '/trade'
    | '/vaults/create'
    | '/invest/'
    | '/portfolio/'
    | '/vaults/'
    | '/vaults/$id/edit'
    | '/invest/vaults/'
    | '/vaults/$id/'
    | '/invest/vaults/$id/'
  fileRoutesById: FileRoutesById
}
export interface RootRouteChildren {
  IndexRoute: typeof IndexRoute
  PayoutRoute: typeof PayoutRoute
  SettingsRoute: typeof SettingsRoute
  TradeRoute: typeof TradeRoute
  VaultsCreateRoute: typeof VaultsCreateRoute
  InvestIndexRoute: typeof InvestIndexRoute
  PortfolioIndexRoute: typeof PortfolioIndexRoute
  VaultsIndexRoute: typeof VaultsIndexRoute
  VaultsIdEditRoute: typeof VaultsIdEditRoute
  InvestVaultsIndexRoute: typeof InvestVaultsIndexRoute
  VaultsIdIndexRoute: typeof VaultsIdIndexRoute
  InvestVaultsIdIndexRoute: typeof InvestVaultsIdIndexRoute
}

declare module '@tanstack/react-router' {
  interface FileRoutesByPath {
    '/': {
      id: '/'
      path: '/'
      fullPath: '/'
      preLoaderRoute: typeof IndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/payout': {
      id: '/payout'
      path: '/payout'
      fullPath: '/payout'
      preLoaderRoute: typeof PayoutRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/settings': {
      id: '/settings'
      path: '/settings'
      fullPath: '/settings'
      preLoaderRoute: typeof SettingsRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/trade': {
      id: '/trade'
      path: '/trade'
      fullPath: '/trade'
      preLoaderRoute: typeof TradeRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/invest/': {
      id: '/invest/'
      path: '/invest'
      fullPath: '/invest/'
      preLoaderRoute: typeof InvestIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/portfolio/': {
      id: '/portfolio/'
      path: '/portfolio'
      fullPath: '/portfolio/'
      preLoaderRoute: typeof PortfolioIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/vaults/': {
      id: '/vaults/'
      path: '/vaults'
      fullPath: '/vaults/'
      preLoaderRoute: typeof VaultsIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/vaults/create': {
      id: '/vaults/create'
      path: '/vaults/create'
      fullPath: '/vaults/create'
      preLoaderRoute: typeof VaultsCreateRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/invest/vaults/': {
      id: '/invest/vaults/'
      path: '/invest/vaults'
      fullPath: '/invest/vaults/'
      preLoaderRoute: typeof InvestVaultsIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/vaults/$id/': {
      id: '/vaults/$id/'
      path: '/vaults/$id'
      fullPath: '/vaults/$id/'
      preLoaderRoute: typeof VaultsIdIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/vaults/$id/edit': {
      id: '/vaults/$id/edit'
      path: '/vaults/$id/edit'
      fullPath: '/vaults/$id/edit'
      preLoaderRoute: typeof VaultsIdEditRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/invest/vaults/$id/': {
      id: '/invest/vaults/$id/'
      path: '/invest/vaults/$id'
      fullPath: '/invest/vaults/$id/'
      preLoaderRoute: typeof InvestVaultsIdIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
  }
}

const rootRouteChildren: RootRouteChildren = {
  IndexRoute: IndexRoute,
  PayoutRoute: PayoutRoute,
  SettingsRoute: SettingsRoute,
  TradeRoute: TradeRoute,
  VaultsCreateRoute: VaultsCreateRoute,
  InvestIndexRoute: InvestIndexRoute,
  PortfolioIndexRoute: PortfolioIndexRoute,
  VaultsIndexRoute: VaultsIndexRoute,
  VaultsIdEditRoute: VaultsIdEditRoute,
  InvestVaultsIndexRoute: InvestVaultsIndexRoute,
  VaultsIdIndexRoute: VaultsIdIndexRoute,
  InvestVaultsIdIndexRoute: InvestVaultsIdIndexRoute,
}
export const routeTree = rootRouteImport
  ._addFileChildren(rootRouteChildren)
  ._addFileTypes<FileRouteTypes>()

import type { getRouter } from './router.tsx'
import type { createStart } from '@tanstack/react-start'
declare module '@tanstack/react-start' {
  interface Register {
    ssr: true
    router: Awaited<ReturnType<typeof getRouter>>
  }
}
