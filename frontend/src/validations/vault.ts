...
export const createVaultSchema = z.object({
  ...
  minRaiseAmount: z.coerce.number().min(0, 'Min raise must be ≥ 0'),
  minRaiseUnit: z.enum(['SOL', 'USDC', 'USDT']),
  acceptedAssets: z.array(z.string()).min(1, 'Select at least one accepted asset'),
  minInvestment: z.coerce.number().min(0.001, 'Min investment must be ≥ 0.001'),
  lockupPeriodValue: z.coerce.number().min(0, 'Lockup period must be ≥ 0'),
  lockupPeriodUnit: z.enum(['hours', 'days']),
  managementFeePercent: z.coerce.number().min(0, 'Min 0%').max(15, 'Management fee cannot exceed 15%'),
  feeWithdrawalPeriod: z.enum(['weekly', 'monthly', 'quarterly', 'yearly']),
  performanceFeePercent: z.coerce.number().min(0, 'Min 0%').max(20, 'Performance fee cannot exceed 20%'),
