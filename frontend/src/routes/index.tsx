...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: generateMetadata({
      title: "Dashboard",
      description:
        "Overview of your Solana vault portfolio, active investments, and performance metrics.",
      path: "/",
    }),
  }),
...
function DashboardPage() {
  ...
  // Real-time channel subscriptions for dashboard & active vaults
  const dashboardChannels = useMemo(() => {
    const channels = ["dashboard", "global:activity", "global:leaderboard"];
    vaults.slice(0, 30).forEach((v) => {
      if (v.id)
        channels.push(
          `vault:${v.id}:portfolio`,
          `vault:${v.id}:activity`,
          `vault:${v.id}`,
        );
      if (v.address && v.address !== v.id)
        channels.push(`vault:${v.address}:portfolio`, `vault:${v.address}`);
    });
    return channels;
  }, [vaults]);

  useRouteWsChannel(dashboardChannels, walletAddress);
  ...
  const managerTVL = useMemo(() => {
    return managerVaults.reduce((sum, v) => sum + (v.tvl || 0), 0);
  }, [managerVaults]);

  const totalTVL = vaults.reduce((sum, v) => sum + (v.tvl || 0), 0);
  const activeVaultCount = vaults.filter((v) => v.status === "Active").length;
  const managerVaultCount = managerVaults.length;
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
            <Card className="relative flex flex-col justify-between overflow-hidden p-5 border-border-subtle">
              ...
              <div className="mt-4">
                ...
                <p className="mt-1 text-xs text-text-tertiary">
                  Across all active Solana vaults
                </p>
              </div>
            </Card>
            ...
            <Card className="relative flex flex-col justify-between overflow-hidden p-5 border-border-subtle">
              ...
              <div className="mt-4">
                <div className="text-3xl font-bold tracking-tight text-text-primary">
                  {vaultsLoading
                    ? "..."
                    : `${activeVaultCount} / ${vaults.length}`}
                </div>
                <p className="mt-1 text-xs text-text-tertiary">
                  Non-custodial strategies available
                </p>
              </div>
            </Card>
            ...
            <Card className="relative flex flex-col justify-between overflow-hidden p-5 border-border-subtle">
              ...
              <div className="mt-4">
                ...
                <p className="mt-1 text-xs text-text-tertiary">
                  Live Devnet low-slippage trade execution
                </p>
              </div>
            </Card>
          </div>
          ...
          <div className="flex h-56 flex-col items-center justify-center rounded-2xl border border-border-subtle text-center p-6">
            ...
            <p className="mt-1 max-w-sm text-xs text-text-secondary">
              Deposit into high-performing vaults or launch your own fund on
              Solana.
            </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
            <Card className="relative flex flex-col justify-between overflow-hidden p-5 border-border-subtle">
              <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-text-tertiary">
                <div className="flex items-center gap-2">
                  <Coins className="size-4 text-primary-gold" />
                  <span>
                    {isManager ? "Managed TVL" : "Total Platform TVL"}
                  </span>
                </div>
                <span className="text-[10px] text-text-muted">Live</span>
              </div>
              <div className="mt-3">
                <div className="text-3xl font-bold tracking-tight text-text-primary">
                  {vaultsLoading
                    ? "..."
                    : `$${(isManager ? managerTVL : totalTVL).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
                </div>
                <p className="mt-1 text-xs text-text-tertiary">
                  {isManager
                    ? `${managerVaultCount} vault(s) under management`
                    : "Across all Solana vaults"}
                </p>
              </div>
            </Card>
            ...
            <Card className="relative flex flex-col justify-between overflow-hidden p-5 border-border-subtle">
              <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-text-tertiary">
                ...
                {isManager && (
                  <Link to="/vaults/create">
                    <button
                      type="button"
                      className="inline-flex items-center gap-1 text-[11px] font-bold text-primary-coral hover:underline cursor-pointer"
                    >
              ...
              <div className="mt-3">
                ...
                <p className="mt-1 text-xs text-text-tertiary">
                  {isManager
                    ? "Vaults under your authority"
                    : "Vaults with active positions"}
                </p>
              </div>
            </Card>
            ...
            <Card className="relative flex flex-col justify-between overflow-hidden p-5 border-border-subtle">
              ...
              <div className="mt-3">
                ...
                <p className="mt-1 text-xs text-text-tertiary">
                  Low-latency oracle updates enabled
                </p>
              </div>
            </Card>
          </div>
          ...
          {!isManager && wallet.publicKey && (
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between">
                ...
                <Link
                  to="/portfolio"
                  className="text-xs text-primary-coral hover:underline font-semibold"
                >
                  View full portfolio →
                </Link>
              </div>
