...
export function ProgressMetricCard({
  ...
  const sz = SIZES[size];
  const shell = `relative flex ${sz.minH} w-full flex-col overflow-hidden rounded-xl border border-border-subtle bg-bg-elevated/60 backdrop-blur-2xl shadow-[0_2px_10px_rgba(0,0,0,0.04)] ${className}`;
      ...
      <div className={shell} aria-busy="true">
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          <div className="flex items-center justify-between">
            <div className="h-5 w-32 animate-pulse rounded bg-bg-inset" />
            <div className="h-5 w-24 animate-pulse rounded bg-bg-inset" />
          </div>
          <div className="mt-6 h-14 w-48 animate-pulse rounded-xl bg-bg-inset" />
          <div className="mt-auto h-24 w-full animate-pulse rounded-xl bg-bg-inset/50" />
        </div>
        <div className={`border-t border-border-subtle ${sz.footer}`}>
  ...
  if (!hasData) {
    return (
      <div className={shell}>
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          <h3 className={`${sz.title} font-semibold tracking-tight text-text-primary`}>{title}</h3>
  ...
  return (
    <div className={shell}>
      <div className="absolute inset-y-0 right-0 z-0" style={{ width: `${REGION_W}%` }}>
        ...
        />
        <div
          className="absolute inset-0 text-text-primary/[0.13]"
          style={{
      ...
      <div className={`pointer-events-none relative z-10 flex flex-1 flex-col ${sz.pad}`}>
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3.5 text-[14px]">
            <span className="flex items-center gap-1 font-medium pointer-events-auto" style={{ color: color.text }}>
        ...
        <div
          className={`mt-5 ${sz.headline} font-medium leading-none tracking-tight text-text-primary`}
        >
          {displayTotal}
        </div>
      </div>
      ...
      <div
        className={`relative z-10 flex items-center justify-between gap-4 border-t border-border-subtle bg-bg-surface/60 backdrop-blur-md ${sz.footer} text-[14px]`}
      >
        <div>
        ...
        {showStats && (
          <div className="flex items-center gap-2.5 text-[12px] text-text-secondary">
            <span>
              <span className="font-medium text-text-primary/80">{fmtCompact(stats.peak)}</span> peak
            </span>
            <span className="opacity-40">·</span>
            <span>
              <span className="font-medium text-text-primary/80">{fmtCompact(stats.low)}</span> low
            </span>
            <span className="opacity-40">·</span>
            <span>
